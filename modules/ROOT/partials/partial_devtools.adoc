...
|===
| *Coding in Cairo* | _VsCode_ + _Cairo plugin_ +
or +
_Remix_ + _Cairo plugin_
| *Building* | _Scarb_
| *Testing and debugging* | _Foundry (Forge)_ +
or +
_Hardhat_ + _Starknet plugin (JavaScript only)_
| *Running on a devnet* |_Scarb_ for compiling and packaging +
_starknet-devnet_/_starknet-devnet-rs (local node)_ +
or +
_Katana (local node)_
| *Contract development* | _Foundry (Cast)_
or
_Starkli_
| *Package delivery* +
(Optional) | _Scarb_
...
The Starknet Foundry Github repo has a link:https://github.com/foundry-rs/starknet-foundry/#roadmap[roadmap] showing new and in-development features, although it’s not clear if the checkmarks indicate features that are already implemented, or in active development.
...
Get started with the Starknet Remix plugin at link:https://github.com/groksmith/starkware-remix-plugin[the Starknet Remix plugin’s Github repo].
